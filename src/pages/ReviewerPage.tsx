...
export const ReviewerPage: React.FC = () => {
  ...
  const handleClearAnswer = (id: string) => {
    setUserAnswers((prev) => {
      const next = { ...prev };
      delete next[id];
      return next;
  ...
  return (
    <div className="py-6 space-y-10">
      ...
      {showControls ? (
        <div className="relative z-10 p-4 sm:p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-md space-y-4 print:hidden transition-all">
          ...
          <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                Reviewer Controls & Filters
              </span>
            </div>

            <button
              onClick={() => setShowControls(false)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-750 text-slate-700 dark:text-slate-300 text-xs font-bold transition-colors border border-slate-200 dark:border-slate-700 shadow-xs"
              title="Close toolbar to maximize reading space"
              aria-label="Close controls"
            >
              <X className="w-3.5 h-3.5 text-rose-500" />
              <span>Close / Hide Controls</span>
            </button>
          </div>
          ...
          <div className="pt-2 border-t border-slate-100 dark:border-slate-800/80 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] text-slate-400 hidden sm:inline">Scroll horizontally for all 12 topics →</span>
            </div>
            ...
            <div className="sm:hidden">
              <select
                ...
                className="w-full px-3 py-2.5 rounded-xl text-xs bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white font-medium focus:ring-2 focus:ring-cyan-500"
              >
                <option value="all">All 12 Topics ({CIT220_REVIEWER_QUESTIONS.length} items)</option>
            ...
            {/* Tablet & Desktop horizontally scrollable pill rail */}
            <div className="hidden sm:flex items-center gap-1.5 overflow-x-auto pb-1.5">
              <button
        ...
        /* Collapsed Minimal Toolbar Pill */
        <div className="flex items-center justify-between p-3.5 px-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm print:hidden">
          <div className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400 truncate pr-2">
            <SlidersHorizontal className="w-4 h-4 text-cyan-600 dark:text-cyan-400 shrink-0" />
            <span className="truncate">
              Showing: <strong className="text-slate-900 dark:text-white capitalize">{selectedType.replace('_', ' ')}</strong> • <strong className="text-slate-900 dark:text-white">{selectedTopicId === 'all' ? 'All 12 Topics' : CIT220_REVIEWER_TOPICS.find((t) => t.id === selectedTopicId)?.title || selectedTopicId}</strong> ({filteredQuestions.length} questions)
            </span>
          </div>

          <button
            onClick={() => setShowControls(true)}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-cyan-600 hover:bg-cyan-700 text-white text-xs font-bold transition-colors shrink-0 shadow-xs"
          >
            <span>Show Filters & Tools</span>
...
// ==========================================
// HELPER: FUZZY CORRECTION MATCHER
// ==========================================
const isUserCorrectionMatch = (userTyped: string, target: string): boolean => {
  if (!userTyped || !target) return false;
  const clean = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, '').trim();
  const cUser = clean(userTyped);
  const cTarget = clean(target);
  if (cUser === cTarget) return true;
  if (cTarget.includes(cUser) && cUser.length >= 3) return true;
  if (cUser.includes(cTarget) && cTarget.length >= 3) return true;
  return false;
};
...
const ModifiedTrueFalseCard: React.FC<MTFCardProps> = ({
  ...
  // Local state for changing / typing correction when FALSE
  const [typedCorrection, setTypedCorrection] = useState('');
  ...
  const handleCorrectionSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!typedCorrection.trim()) return;
    setIsCorrectionChecked(true);
  };
  ...
  const isCorrectionMatch = isUserCorrectionMatch(typedCorrection, question.correction || '');
  ...
  return (
    <div className="p-6 sm:p-7 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-md dark:shadow-xl space-y-5 transition-all">
      ...
      <p className="text-xs text-slate-500 italic">
        Direction: If the underlined term makes the statement true, choose TRUE. If false, choose FALSE and change the underlined term to make the statement true.
      </p>
      ...
      {userAnswer === false && !question.isTrue && (
        <div className="p-4 sm:p-5 rounded-2xl bg-slate-100 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 space-y-3.5 shadow-sm">
          ...
          <form onSubmit={handleCorrectionSubmit} className="flex flex-col sm:flex-row gap-2">
            <input
              type="text"
              value={typedCorrection}
              onChange={(e) => {
                setTypedCorrection(e.target.value);
                setIsCorrectionChecked(false);
              }}
            ...
            >
              {isCorrectionMatch ? (
                <>
                  ...
                  <div>
                    <p className="font-bold">Correct Replacement! Excellent recall.</p>
                    <p className="mt-0.5 text-[11px]">
                      You changed "{question.underlinedTerm}" to <strong className="underline font-bold">"{question.correction}"</strong>.
                    </p>
                  </div>
                </>
              ) : (
                <>
                  ...
                  <div>
                    <p className="font-bold">Not quite the verbatim term.</p>
                    <p className="mt-0.5 text-[11px]">
                      You entered: "{typedCorrection}". Expected verbatim term: <strong className="underline font-bold text-emerald-800 dark:text-emerald-300">"{question.correction}"</strong>
                    </p>
                  </div>
                </>
              )}
        ...
        <div className="flex items-center gap-2">
          {isAnswered && (
            <button
              onClick={onClear}
              className="text-xs text-slate-500 hover:text-slate-800 dark:hover:text-slate-300 underline font-medium"
              title="Clear selection and try again"
            >
              Reset Answer
            </button>
          )}
